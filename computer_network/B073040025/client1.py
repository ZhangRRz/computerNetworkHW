...
thread_lock_connect_part = 0


def create_new_client(c_num):
    global thread_lock_connect_part
    # while(thread_lock_connect_part):
    #     pass
    # thread_lock_connect_part = 1
    ...
    while job == 'y':
        ...
        job_count += 1
        if(PacketType == "calc"):
            request_packet_data += PacketType+"\n"
            request_packet_data += input("Input math expression: \n")
            job_type.append("calc")

        elif(PacketType == "video"):
            request_packet_data += PacketType+"\n"
            request_packet_data += input("Input which video you want: \n")
            job_type.append("video")
        
        elif(PacketType == "dns"):
            request_packet_data += PacketType+"\n"
            request_packet_data += input("Input which IP of domain name you want get: \n")
            job_type.append("dns")
        else:
            job_count -= 1
    ...
    print("---------3 way handshake start--------------")
    tcp = tcppacket.TCPPacket(flags_syn = 1,
                              seq=seq)
    tcp.assemble_tcp_feilds()
    print("SYN send to (IP,port): ",udp_host," , ",udp_port,
          " with seq: ",seq)
    ...
    unpack = struct.unpack('!HHLLBBHHH', data[:struct.calcsize('!HHLLBBHHH')])
    print("SYN/ACK receive from (IP,port): ",senderAddr,
          " with ack seq: ",unpack[3]," seq: ",unpack[2])
    ...
    tcp = tcppacket.TCPPacket(data=request_packet_data.encode('utf-8'),
                              seq=seq,
                              ack_seq=ack_seq)
    tcp.assemble_tcp_feilds()
    print("request send to (IP,port):",senderAddr)
...
def maybe_make_packet_error():
    if(random.randint(1, 1000000) < 200000):
        # make packet error
        return 1
    return 0