...
def handle_request(data, client_address, threadnum):
    ...
    # See header to know what this packet want to do
    # unpackdata[8] = what packet is
    if(unpackdata[8] == 0):
        print("receive a text packet from client")
        print("content is : "+msg)
        reply = str("got text msg").encode('utf-8')
        fin_falg = 1
    ...
    while True:
        ...
        seq += 1
        packet_count += 1
        # back up a correct packet
        tcp_backup = tcppacket.TCPPacket(data=reply,
                                seq=seq, ack_seq=ack_seq,
                                flags_fin=fin_falg,)
        tcp_backup.assemble_tcp_feilds()
        send_packet_temp.append(tcp_backup.raw)


        if(packet_count == 3 or fin_falg == 1):
            packet_count = 0
            # receive ACK
        ...
        if(unpackdata[3] == seq):
            ...
            pass
        elif(unpackdata[3] < seq):
            seq -= len(send_packet_temp)
            for i in range(len(send_packet_temp)):
                thread_sock.sendto(send_packet_temp[i], client_address)
...
def maybe_make_packet_error():
    # if(random.randint(1, 1000000) < 10000):
    #     # make packet error
    #     return 1
    return 0
